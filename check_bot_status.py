...
up_count = 0
down_svcs = []
# No more "stale" special-case here -- that existed only because
# mesh_bot/meshtasticd's real status came from a local LXC check whose
# absence had to be inferred via a separate lxc-monitor proxy. Now a
# failed public-API request directly marks them down (see the checks{}
# construction above), so `down` here is already an honest, current
# answer, not a possibly-stale one.
for svc in SERVICES:
    st = state.get(svc, {})
